  ...
  Category.findById(req.body.categoryId)
    .then((data)=> {
        ...
        if(data.recipes.some(e=>e.toString() === req.body.recipeId)){
            return res.json({ result: false, error: 'Recipe already registred in this category' })
        }
        data.recipes.push(new mongoose.Types.ObjectId(req.body.recipeId));
        data.save()
         .then((data)=> { res.json({ result: true, message: `Recipe added to ${data.name}`});
         });
    })
      .catch(err => { 
        console.log(err);
...
router.delete('/removeRecipeFromCategory', (req,res)=>{
  Category.findById(req.body.categoryId)
    .then((data)=> {
        ...
        data.recipes = data.recipes.filter(e=>e.toString() !== req.body.recipeId.toString())
        data.save()
         .then((data)=> { res.json({ result: true, message: `Recipe removed from ${data.name}`});
         });
    })
    .catch(err => { 
        console.log(err);
